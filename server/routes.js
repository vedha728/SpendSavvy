...
import { insertExpenseSchema, updateExpenseSchema, insertDebtSchema, updateDebtSchema } from "../shared/schema.ts";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/expenses", async (req, res) => {
        try {
            ...
            if (startDate && endDate) {
                expenses = await storage.getExpensesByDateRange(new Date(startDate), new Date(endDate));
            }
            else if (category) {
    ...
    app.get("/api/expenses/analytics/stats", async (req, res) => {
        try {
            ...
            const selectedMonth = req.query.month ? parseInt(req.query.month) : today.getMonth();
