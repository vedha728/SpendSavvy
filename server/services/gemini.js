...
export async function processExpenseQuery(userMessage) {
    try {
        ...
        if (message.includes('debt') || message.includes('owe') || message.includes('lend') || message.includes('borrow')) {
            ...
            if (friendMatch && amountMatch) {
                ...
                const descriptionWords = words.filter(word => !word.match(/\d/) &&
...
export async function generateExpenseInsights(expenses, query) {
    try {
